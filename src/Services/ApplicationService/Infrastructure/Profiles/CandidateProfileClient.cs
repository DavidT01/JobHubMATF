...
public sealed class CandidateProfileClient(HttpClient httpClient, IHttpContextAccessor contextAccessor)
    : ICandidateProfileReader
{
        ...
        string userId, CancellationToken cancellationToken)
    {
        using var request = new HttpRequestMessage(HttpMethod.Get,
            $"api/candidate-profiles/{Uri.EscapeDataString(userId)}");
        ...
        try
        {
            ...
            var profile = await response.Content.ReadFromJsonAsync<CandidateProfileReference>(cancellationToken);
